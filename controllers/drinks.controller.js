...
module.exports.drinksController = {
    ...
    postDrink: (req, res) => {
        ...
        }).then(()=>{res.json('Напиток добавлен')}).catch((err)=>{res.json('Ошибка')})
    },
    ...
    patchDrink: (req, res) => {
        Drinks.findByIdAndUpdate(
            req.params.id, {
                ...
                size: req.body.size,
                about: req.body.about
            }).then(()=>{res.json('Напиток изменен')}).catch((err)=>{res.json('Ошибка')})
    },

    deleteDrinks: (req, res) => {
        Drinks.findByIdAndRemove(req.params.id).then(()=>(res.json('Напиток удален'))).catch((err)=>{res.json('Ошибка')})
    },

    getDrinksById: (req, res) => {
        Drinks.findById(req.params.id).then((data)=>{res.json(data)}).catch((err)=>{res.json('Ошибка')})
    },
